...
passport.use(new FacebookStrategy({
        ...
        callbackURL : 'http://192.168.3.148:8080/auth/facebook/callback',    // 8080 : port that server listening
...
app.get('/addChild', function(req, res){
    ...
    var sql = "select count(child_name) cnt from childs where child_name='" + cname + "'";
    global.mysql.query(sql, function(err, rows){
        if( rows[0].cnt > 0)
            res.json({result:'exist'});
        else{
            global.mysql.query('insert into childs set ?',{child_name: cname},
                function(err, result){
...
app.get('/deleteChild', function(req, res){
    ...
    var sql = "delete from childs where child_id='" + cid + "'";
    global.mysql.query(sql, function(err, rows){
...
app.get('/addFaculty', function(req, res){
    ...
    var sql = "select count(user_name) cnt from users where grade='2' and facility='" + facility +"'";
    global.mysql.query(sql, function(err, rows){
        if( rows[0].cnt > 0)
            res.json({result:'exist'});
        else{
            global.mysql.query('insert into users set ?',{user_name: fname, grade:'2', facility: facility},
                function(err, result){
                    res.json({result: 'success'});
                }
            )
...
app.get('/deleteFaculty', function(req, res){
    var sql = "delete from users where grade='2'";
    global.mysql.query(sql, function(err, rows){
        if( !err ) res.json({result:'success'});
        else res.json({result:'fail'});
    })
...
// Email doubling check
app.post('/check-double-email', function(req, res){
    var mail = req.body.mail;
    var sql = "select count(user_id) cnt from users where user_email='" + mail + "'";
    global.mysql.query(sql, function(err, rows){
        res.json({result : rows[0].cnt});
    })
})
// save User Data(Email, password, grade)
app.post('/save-epg', function(req,res){
    var mail = req.body.mail, pwd = req.body.pwd,
        grade = req.body.grade, facility = req.body.facility;
    var sql = "insert into users set ?";
    global.mysql.query('INSERT INTO users SET ?',
        {user_email:mail, user_pwd:pwd, grade: grade, facility:facility},
        function(err, result){
        if(err)   res.json({uid : '-1', grade: grade});
        else {
            res.json({uid : result.insertId, grade: grade});
...
// For admin-setting menu, get name, position and facility of user
app.get('/getUserData', function(req,res){
    var uid = req.session.user_id;
    var sql = "select user_name, grade, facility from users where user_id='" + uid + "'";
    global.mysql.query(sql, function(err, rows){
...
// call signup parent information page
app.get('/signup-parent-info', function(req, res){
    res.render('signup_parent_inform');
})

// call signup child information page
app.get('/signup-child-inform', function(req, res){
    res.render('signup_child_inform');
})

// call signup medical information page
app.get('/signup-child-medical', function(req, res){
    res.render('signup_medical');
})

// call signup medical information page
app.get('/signup-emergency-contact', function(req, res){
    res.render('signup_emergency_contact');
})
...
// GET  STATES  NAMES
app.get('/get-state-names', function(req, res){
    var sql= 'select name from states order by id';
    global.mysql.query(sql, function (err, rows) {
        res.json(rows);
    })
})

// When Save child Profile in parent signup page
app.post('/save-child-profile', function(req, res) {
    // console.log(req);
    var cid= req.body.cid,
        nickname= req.body.nickname, dob= req.body.dob,
        street= req.body.street, city= req.body.city,
        state= req.body.state, facility= req.body.facility,
        uid= req.body.uid, zip = req.body.zip;

    // if can not find user, show message
    var sql = "update childs set nickname=?,birthday=?,address=?,zipcode=?,city=?,state=?,facility=? where child_id=?";
    var data = [nickname, dob, street, zip, city, state, facility, cid];

    global.mysql.query(sql, data, function (err, rows) {
        if (err) { console.error(err); res.json({result:err}); }
 //       var cid = rows.insertId;
        sql = mysql.format('update users set child_id=? where user_id=?', [cid, uid]);
        global.mysql.query(sql, function(err, result){
            res.json({cid : cid, uid : uid});
        })
    });
});
...
// save parent profile when sign up
app.post('/save-parent-profile', function(req, res){
    var sql = "update users set user_name=?, gender=?, phonenumber=?," +
        "address=?, city=?, state=?, zipcode=?, employer=?, " +
        "workphone=?, workaddress=?,workcity=?, workzipcode=?, workstate=? where user_id=?";
    var data = [req.body.name1 + ' ' + req.body.name2, req.body.gender, req.body.phone,
        req.body.street, req.body.city, req.body.state, req.body.zip,
        req.body.employer, req.body.workphone, req.body.workstreet,
        req.body.workcity, req.body.workzip, req.body.workstate, req.body.uid];
...
app.get('/get-childID', function(req, res){
    var uid = req.session.user_id;

    global.mysql.query("select child_id from users where user_id=?", [uid], function(err, rows){
        console.log(rows);
        if (err) console.log(err);
        else  if(rows.length>0)  res.json({cid : rows[0].child_id});
        else res.json({cid : 0});
    })
})
...
app.get('/calendar', function(req, res){
    res.render('calendar', { nname: req.session.nickname, grade: req.session.grade });
});
...
app.get('/getChildAction', function(req, res){
    var cid = req.query.cid;

    var cdate = new Date(), cds = cdate.getFullYear()+'-'+(cdate.getMonth()+1)+'-'+cdate.getDate();
    var sql = "select cid,id,btime,action_id,action_content,action_note from report where cid='"+cid+"' and rdate='"+cds+"' order by id";
    global.mysql.query(sql, function(err, rows){
        if(err) res.json({result : err});
        if(rows != undefined) res.json(rows);
    })
})
...
app.get('/search-event', function(req, res){
    ...
    var sql = "SELECT * FROM (SELECT c.*, d.`child_name` FROM " +
        "(SELECT b.user_id, b.`child_id`, b.`user_name`,a.`event`, a.`mdate`, a.`mtime`, a.`ndate`, a.`ntime`, a.`note` FROM events AS a " +
...
app.post('/save-event', function(req, res){
    var mdate = req.body.mdate, mtime = req.body.mtime,
        ndate = req.body.ndate, ntime = req.body.ntime,
        event = req.body.event, note = req.body.note,
        user_id = req.body.user_id;

    event = event.replace(/'/gi, "`");
    note = note.replace(/'/gi, "`");

    var sql = "insert into events (mdate, mtime,";
    if(ndate != '') sql += " ndate,";
    if(ntime != '') sql += " ntime,";
    sql += "event, note, user_id) values('"+mdate+"','"+mtime+"',";
    if(ndate != '') sql += "'"+ndate+"',";
    if(ntime != '') sql += "'"+ntime+"',";
    sql += "'"+event+"','" + note+"','" + user_id +"')";
...
app.post('/edit-note', function(req, res){
    ...
    global.mysql.query(sql, function(err, rows){
        ...
        res.json({result: "success", rid:rid, rdate:rdate, btime:btime, action_id: act_id});
    })
});
...
app.post('/new-note', function(req, res){
    ...
    global.mysql.query(sql, function(err, rows){
        ...
        sql = "select id rid, cid,btime,action_id from report where cid='" + cid + "' and rdate='" + rdate + "' and btime='" + bt + "' and action_id='"+action_id+"'";
...
app.post('/new-photo-register', function(req, res){
    ...
    var sql = "insert into report (cid,action_id,action_content,author, rdate,btime,imgsrc) " +
        "value('"+cid+"','11','" + cont + "','" + author+"','"+bdate+"','"+ bt + "','" + url + "')";

    global.mysql.query(sql, function (err, rows) {
        if (err) { res.json({state : "False"}); }
        sql = "select id rid, cid,btime,action_id,action_content,imgsrc from report where cid='" + cid + "' and rdate='" + bdate + "' and btime='" + bt + "' and action_id='11'";
        global.mysql.query(sql, function(err, rows){
            res.json(rows);
        })
    })
...
app.post('/delete-note', function(req, res){
    ...
    global.mysql.query(sql, function(err, rows){
        ...
        res.json({result: "success", rid : rid});
    })
});
...
app.post('/new-bottle', function(req, res){
    ...
    var sql = "insert into report (cid, rdate, btime, action_id, action_content, author) values('" +
        cid + "','" + rdate + "','"+ bt +"','" + action_id + "','" + action_content + "','" + author + "')";
    console.log(sql);
    ...
    global.mysql.query(sql, function(err, rows){
        ...
        var sql = "select id rid, cid, btime, action_id, action_content from report where cid='" + cid + "' and rdate='" + rdate + "' and btime='" + bt + "' and action_id='" + action_id + "'";
        console.log(sql);
        ...
        global.mysql.query(sql, function(err, rows) {
            if(rows.length > 0)
                ...
                res.json({result: "false", note_id: "-1"});
        })
    })
});
...
app.post('/edit-bottle', function(req, res){
    ...
    var rdate = req.body.rdate;
    var btime = req.body.btime;
    var action = req.body.action_content;
    var action_id = req.body.action_id;
    ...
    var bt =btime.toLowerCase(), ss = bt.split(' ');
    if (bt.includes('m')) bt = ss[0] + ':00';

    var sql = "update report set rdate='"+rdate+"', btime='"+bt+"',action_id='"+action_id+"', action_content='"+action+"' where id='"+rid+"'";

    global.mysql.query(sql, function(err, rows){
        if(err)
            res.json({ result:'Inserting failure' });
        else
            res.json({result:'success', rid : rid, rdate: rdate, btime: btime, action:action, action_id: action_id});
    })
});
// when inserting new comment
app.post('/new-comment', function(req, res){
    ...
    var sql = "insert into report (cid, rdate, btime, action_id, action_note, author) values('" +
        cid + "','" + rdate + "','"+ bt +"','" + action_id + "','" + note + "','" + author + "')";
    console.log(sql);
    ...
    global.mysql.query(sql, function(err, rows){
        ...
        var sql = "select id rid, cid, btime, action_id, action_note from report where cid='" + cid + "' and rdate='" + rdate + "' and btime='" + bt + "' and action_id='" + action_id + "'";
        console.log(sql);
...
app.post('/edit-comment', function(req, res){
    ...
    var sql = "update report set rdate='"+rdate+"', btime='"+bt+"',action_id='"+action_id+"', action_note='"+note+"' where id='"+rid+"'";
...
app.get('/get-event', function(req, res){
    ...
    var sql = "SELECT mdate, mtime,ndate,ntime, event, note, grade FROM " +
        "(SELECT a.*, b.`grade` FROM events AS a LEFT JOIN users AS b ON a.`user_id` = b.`user_id`) AS a" +
        " where mdate between '" + first + "' and '" + last + "' order by grade desc";
    // var sql = "select mdate, mtime, event, note, grade from events where mdate between '" + first + "' and '" + last + "' order by mdate";
    global.mysql.query(sql, function(err, rows){
        if (err) {
            console.error(err);
            throw err;
...
app.post('/select_during', function(req, res){ //{during: $('#during').val()});
    ...
    var selbegin = req.body.selbegin;
    var selend = req.body.selend;
    ...
    var sql = "select sender, mdate,mtime,msg,imgsrc from message " +
        "where (mdate between '" + bstr + "' and '" + estr + "') " +
        "and in_out='0' order by id desc";
        //"and in_out='0' order by id desc limit " + selbegin + ", 10";
...
app.post('/getSureName', function(req, res){
    var nickname = req.body.nickname;
    var sql = "select user_id, user_name from users where nickname='" + nickname + "'";
    global.mysql.query(sql, function(err, rows){
        //console.log(rows);
        res.json(rows);
    })
...
app.post('/getSureNames', function(req,res){
    var sql = "select user_id, user_name, nickname from users order by user_id";
    global.mysql.query(sql, function(err, rows){
        //console.log(rows);
        res.json(rows);
    })
...
io.sockets.on('connection',function(socket){
    ...
    socket.on('joinroom',function(data) {
        ...
        // broadcast changed room list and user list
        var temp = Object.keys(rooms), roomnames= []; // namelist of all rooms
        var usernames = [];
        var cids = [];  // for getting phonenumber
        temp.forEach(function(room){
            var ur = Object.keys(rooms[room].socket_ids); // all nicknames in that room
            if(ur.length > 0) {
                roomnames.push(room);
                usernames.push(ur);
                cids.push(rooms[room].cid);
            }
        })

        roomnames.forEach( function(room){
            console.log(room);
            io.sockets.in(room).emit('roomuserlist', {rooms: roomnames, users:usernames, cids:cids});
        })
    });
